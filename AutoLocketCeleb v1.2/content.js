class LocketCelebHelper {
    constructor() {
        ...
        this.timerUpdateInterval = null; // Thêm interval để cập nhật timer liên tục
    ...
    async startAutomation(celeb, autoRandom, timerEnabled, timerDuration) {
        ...
        // Không khởi tạo timer ngay ở đây, chờ đến khi nhấn nút "Bắt đầu"
        this.timerEndTime = 0;
    ...
    resumeAutomation() {
        if (this.isRunning) {
            // Kiểm tra nếu timer đã hết hạn thì reset
            ...
            // Chỉ khởi động timer update nếu timer đã được bắt đầu (timerEndTime > 0)
    ...
    // Hàm mới để cập nhật timer liên tục
    startTimerUpdate() {
    ...
    handleTimerEnd() {
        ...
        // Reset về celeb ban đầu và thời gian mới
        this.currentCeleb = this.initialCeleb;
        this.timerEndTime = Date.now() + this.timerDuration * 60 * 1000;
        this.saveState();
    ...
    async runCycle() {
        ...
        // Bắt đầu bộ đếm thời gian giám sát 5 phút
        ...
        try {
            ...
            if (!shouldContinue) {
                // Hủy bộ đếm thời gian giám sát vì chu trình đã dừng một cách hợp lệ
                if (this.watchdogTimer) clearTimeout(this.watchdogTimer);

                if (this.timerEnabled) {
                    this.updateStatus(`${this.currentCeleb} đã là bạn. Tiếp tục chạy và chờ timer...`);
                    // Không dừng, tiếp tục chạy và chờ timer hoặc tìm celeb khác
                    if (this.autoRandom) {
            ...
            if (this.isRunning) {
                ...
                // Không bao giờ dừng khi có lỗi, luôn thử lại
                setTimeout(() => {
                    if (this.isRunning) {
                        this.runCycle();
                    }
    ...
    stopAutomation() {
        // Chỉ dừng khi được gọi một cách rõ ràng (từ popup hoặc lỗi nghiêm trọng)
        ...
        if (this.watchdogTimer) clearTimeout(this.watchdogTimer);
        this.stopTimerUpdate(); // Dừng timer update
        this.timerEnabled = false;
        this.timerEndTime = 0;
        this.saveState();
    ...
    // Hàm kiểm tra xem có nên tiếp tục chạy không
    shouldContinueRunning() {
        // Luôn tiếp tục nếu đang chạy và có timer
        if (this.isRunning && this.timerEnabled) {
            return true;
        }
        // Tiếp tục nếu đang chạy và có chế độ random
        if (this.isRunning && this.autoRandom) {
            return true;
        }
        // Tiếp tục nếu automation vẫn đang được bật
    ...
    startGeneralObserver() { 
        const generalObserver = new MutationObserver((mutations, observer) => { 
            ...
            if (notificationTitle) { 
                ...
                if (popupContainer) { 
                    const closeButton = Array.from(popupContainer.querySelectorAll('button')).find(btn => btn.textContent.trim().toLowerCase() === 'x' || btn.getAttribute('aria-label')?.toLowerCase().includes('close') || btn.getAttribute('aria-label')?.toLowerCase().includes('đóng')); 
                    if (closeButton) { 
                        closeButton.click(); 
                    } 
                } 
        ...
        if (this.isRunning && this.autoRandom) { 
            ...
            try { 
                ...
                if (availableCelebs.length > 0) { 
                    ...
                    // Không bao giờ dừng automation khi có timer
                    if (!this.timerEnabled) {
                        // Chỉ dừng nếu không có timer
                        this.updateStatus('Không có timer. Dừng automation.');
                        this.stopAutomation(); 
                    } else {
                        // Có timer - tiếp tục chạy, scan lại sau 10 giây
                        setTimeout(() => {
                            if (this.isRunning) {
            ...
            // Không bao giờ dừng, luôn tiếp tục
            setTimeout(() => {
                if (this.isRunning) {
                    this.runCycle();
                }
    ...
    async step4_StartProcess() { 
        ...
        if (startButton) { 
            ...
            // Chỉ bắt đầu timer sau khi nhấn nút "Bắt đầu" thành công
            if (this.timerEnabled && this.timerEndTime === 0) {
                this.timerEndTime = Date.now() + this.timerDuration * 60 * 1000;
                this.startTimerUpdate();
                this.saveState();
    ...
    findCelebrityLink() { 
        ...
        for (const link of links) { 
            if ((link.textContent.includes('Celebrity') || link.href?.includes('celebrity')) && link.offsetParent !== null) return link; 
        } 
        return null; 
    }
    ...
    findCelebrityContainer(name) { 
        ...
        for (const card of allProfileCards) { 
            const nameElement = card.querySelector('div.profile-name'); 
            if (nameElement && nameElement.textContent.trim() === name) return card; 
        } 
        return null; 
    }
    ...
    findElementByText(tag, texts) { 
        ...
        for (const element of elements) { 
            for (const text of texts) { 
                if (element.textContent.includes(text) && element.offsetParent !== null) return element; 
            } 
        } 
        return null; 
    }
