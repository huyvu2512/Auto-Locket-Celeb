...
class LocketCelebHelper {
    ...
    async init() {
        console.log('Locket Celeb Helper initialized v1.6 - HTML Structure Scan');
        
        chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
            this.handleMessage(message);
            // DO NOT return true here. This listener only receives commands, it doesn't send a response.
        });
    ...
    // =================================================================
    // THE DEFINITIVE SCANNING LOGIC BASED ON PROVIDED HTML STRUCTURE
    // =================================================================
    scanCelebs(isManualScan = false) {
        return new Promise(async (resolve) => {
            ...
            // 1. Find all celebrity profile cards on the page.
            const profileCards = document.querySelectorAll('div.profile');

            for (const card of profileCards) {
                // 2. Within each card, find the element containing the name.
                const nameElement = card.querySelector('div.profile-name');
                
                // 3. Within the same card, check if an "Add Friend" button exists.
    ...
    findCelebrityContainer(name) {
        // Find the div.profile that contains a div.profile-name with the correct text
