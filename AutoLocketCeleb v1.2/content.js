class LocketCelebHelper {
    constructor() {
        ...
        this.watchdogTimer = null; // Thêm bộ đếm thời gian giám sát
    ...
    async runCycle() {
        ...
        try {
            ...
            if (this.isRunning) {
                ...
                // Không hủy watchdog ở đây, để nó tiếp tục chạy và tải lại nếu lỗi kéo dài
    ...
    stopAutomation() {
        ...
        if (this.timerId) clearTimeout(this.timerId);
        if (this.watchdogTimer) clearTimeout(this.watchdogTimer); // Hủy watchdog khi dừng
        this.timerEnabled = false;
        this.timerEndTime = 0;
        this.saveState();
    ...
    async handleSuccessAndRestart() { 
        if (this.watchdogTimer) clearTimeout(this.watchdogTimer); // Hủy watchdog vì đã thành công
