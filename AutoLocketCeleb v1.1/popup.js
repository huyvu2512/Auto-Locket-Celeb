...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
        if (message.type === 'STATUS_UPDATE') {
            const isRunningStatus = message.status.includes('Đang chạy...') || message.status.includes('Đã bật giám sát');
            
            if (!hasCycleStarted && isRunningStatus) {
...
async function stopAutomation(showConfirm = true) {
    ...
    if (showConfirm && confirm('Bạn có chắc muốn dừng chu kỳ tự động?')) { stop(); }
...
function showRunningInterface() {
    ...
    if (automationState.timerEnabled) {
        ...
        if (automationState.currentStep.includes('Tìm')) {
            timerCountdown.textContent = 'Đang tìm celeb...';
        } else {
...
async function navigateToCelebPage() { try { const [tab] = await chrome.tabs.query({ active: true, currentWindow: true }); const targetUrl = 'https://locket.binhake.dev/celebrity.html'; if (tab && tab.url && tab.url.startsWith('https://locket.binhake.dev/') && tab.url !== targetUrl) { await chrome.tabs.update(tab.id, { url: targetUrl }); window.close(); } } catch (error) { console.error('Error navigating to celeb page:', error); } }
...
function showSetupInterface() { setupInterface.classList.remove('hidden'); runningInterface.classList.add('hidden'); if(countdownIntervalId) clearInterval(countdownIntervalId); hasCycleStarted = false; }
